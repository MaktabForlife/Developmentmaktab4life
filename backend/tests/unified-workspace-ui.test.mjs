...
assert.match(shell, /action: "switch-context"/);
...
for (const html of [adminHtml, studentHtml]) {
  ...
  assert.match(html, /\/js\/m4l-auth\.js\?v=102\.4/);
  assert.match(html, /\/js\/m4l-shell\.js\?v=102\.4/);
  assert.match(html, /\/js\/m4l-timetable\.js\?v=102\.4/);
  assert.match(html, /\/js\/m4l-progress\.js\?v=102\.4/);
}

console.log("V102.4 unified operational workspace UI tests passed.");
