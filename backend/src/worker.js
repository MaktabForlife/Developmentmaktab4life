...
let googleSheetsAccessTokenCache = {
  key: "",
  token: "",
  expiresAt: 0
};
let googleSheetsAccessTokenPromise = null;
...
async function getGoogleSheetsAccessToken(env) {
  ...
  if (
    googleSheetsAccessTokenCache.key === cacheKey &&
    googleSheetsAccessTokenCache.token &&
    googleSheetsAccessTokenCache.expiresAt > nowMs + 60000
  ) {
    return googleSheetsAccessTokenCache.token;
  }

  if (googleSheetsAccessTokenPromise) {
    const pendingResult = await googleSheetsAccessTokenPromise;
    return pendingResult.accessToken;
  }

  googleSheetsAccessTokenPromise = requestGoogleSheetsAccessToken(config).finally(() => {
    googleSheetsAccessTokenPromise = null;
  });

  const tokenResult = await googleSheetsAccessTokenPromise;
  googleSheetsAccessTokenCache = {
    key: cacheKey,
...
async function readGoogleSheetValues(env, range) {
...
async function updateGoogleSheetValues(env, range, values) {
