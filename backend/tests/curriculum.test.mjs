...
try {
  ...
  const legacySubjects = await postCurriculum(
    "/api/admin/subjects/list",
    adminToken,
    {},
    fallbackEnv
  );
  assert.equal(legacySubjects.response.headers.get("X-M4L-Feature"), "curriculum-read");
  assert.equal(legacySubjects.response.headers.get("X-M4L-Backend"), "apps-script");
  assert.deepEqual(appsScriptPayload, { action: "listSubjects" });

  const legacyTasks = await postCurriculum(
    "/api/admin/tasks/list",
    adminToken,
    { subjectid: "SUB1", activeOnly: true },
    fallbackEnv
  );
  assert.equal(legacyTasks.response.headers.get("X-M4L-Feature"), "curriculum-read");
  assert.equal(legacyTasks.response.headers.get("X-M4L-Backend"), "apps-script");
  assert.deepEqual(appsScriptPayload, {
    action: "listTasks",
    data: { subjectid: "SUB1", activeOnly: true }
  });

  const legacyResources = await postCurriculum(
    "/api/admin/subject-resources/list",
    adminToken,
    { subjectid: "SUB1" },
    fallbackEnv
  ...
  assert.equal(
    legacyResources.response.headers.get("X-M4L-Feature"),
    "curriculum-resources-read"
  );
  assert.equal(legacyResources.response.headers.get("X-M4L-Backend"), "apps-script");
  assert.deepEqual(appsScriptPayload, {
    action: "listSubjectResources",
    subjectid: "SUB1"
  });

  const legacyCreate = await postCurriculum(
  ...
  assert.equal(legacyCreate.response.headers.get("X-M4L-Feature"), "curriculum-write");
  assert.equal(legacyCreate.response.headers.get("X-M4L-Backend"), "apps-script");
  assert.deepEqual(appsScriptPayload, {
    action: "createSubject",
    data: { subjectName: "New Subject" }
  });
} finally {
  globalThis.fetch = originalFetch;
}
