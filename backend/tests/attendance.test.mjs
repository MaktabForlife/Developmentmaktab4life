...
globalThis.fetch = async (input, init = {}) => {
  ...
  return response({ success: true, count: 0, students: [] });
};

try {
  const legacyRead = await postAttendance(
    "/api/attendance/students",
    teacherToken,
    { classgroup: "2" },
    {
  ...
  assert.equal(legacyRead.response.headers.get("X-M4L-Feature"), "attendance-read");
  ...
  const legacyReport = await postAttendance(
    "/api/attendance/report",
    teacherToken,
    {
      startDate: "2026-07-27",
  ...
  assert.equal(legacyReport.response.headers.get("X-M4L-Feature"), "attendance-read");
  ...
  appsScriptPayload = null;
  const legacyWrite = await postAttendance(
    "/api/attendance/submit-absent",
    seniorToken,
    { date: "2026-07-29", absentStudents: [] },
    {
  ...
  assert.equal(legacyWrite.response.headers.get("X-M4L-Feature"), "attendance-write");
